...
def tensor_to_mpo(tensor):
    ...
    ordered from left to right. If the Hamiltonian was expressed in terms of outer-product
    projectors, the first N dimensions of the tensor would correspond to the ket, 
    and the last N dimensions to the bra of the outer product.
    ...
    axis_permutation = list(itertools.chain.from_iterable(zip(range(sites), 
        range(sites, 2 * sites))))
    ...
    for i, (out_dim, in_dim) in enumerate(zip(output_shape, input_shape)):
        if i == 0:
            ...
            # NB: parentheses around indices mean they are grouped into the row or column 
            # of a matrix.
            matrices[i] = matrices[i].reshape((out_dim, in_dim, matrices[i].shape[1]))
        elif i == sites - 1:
            # For the rightmost tensor, the sequence of SVD's leaves a shape 
            # a_{L-1}, (out_dim, in_dim).
            ...
            matrices[i] = matrices[i].reshape((left_bond_dim, 
                out_dim, in_dim, matrices[i].shape[1]))
